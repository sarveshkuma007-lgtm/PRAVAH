...
export function WeatherForecast() {
  ...
  const { weather, hourly, weekly, loading } = useWeather(selectedDamName);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            IMD weather radar, convective precipitation forecasts, and cloudburst early detection.
          </p>
        </div>

        {/* Catchment Basin Selector */}
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-400 font-mono">Catchment:</span>
          <select
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-cyan-950 border border-slate-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <h2 className="text-3xl font-black text-white mt-1">
              ...
              <span className="text-base font-normal text-slate-400 ml-2 font-mono">
                {weather.current.condition}
              </span>
            </h2>
            <p className="text-xs text-slate-300 mt-1">
              {weather.current.location} &bull; Severe convective rain band moving east-northeast
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full md:w-auto">
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-center">
            ...
            <span className="text-[10px] uppercase font-mono text-slate-400 block">24h Rain</span>
            <span className="font-bold text-slate-100 font-mono text-sm">{weather.current.rainfall24h} mm</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-center">
            <Wind className="w-4 h-4 text-blue-400 mx-auto mb-1" />
            <span className="text-[10px] uppercase font-mono text-slate-400 block">Wind</span>
            <span className="font-bold text-slate-100 font-mono text-sm">{weather.current.windSpeed} km/h</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-center">
            <Droplets className="w-4 h-4 text-cyan-400 mx-auto mb-1" />
            <span className="text-[10px] uppercase font-mono text-slate-400 block">Humidity</span>
            <span className="font-bold text-slate-100 font-mono text-sm">{weather.current.humidity}%</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-center">
            <Gauge className="w-4 h-4 text-amber-400 mx-auto mb-1" />
            <span className="text-[10px] uppercase font-mono text-slate-400 block">Barometer</span>
            <span className="font-bold text-slate-100 font-mono text-sm">{weather.current.pressure} hPa</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 rounded-xl bg-slate-900/80 border border-slate-800">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          {weekly.map((day, idx) => (
            ...
            >
              <div>
                <span className="font-bold text-slate-200 block font-mono">{day.day}</span>
